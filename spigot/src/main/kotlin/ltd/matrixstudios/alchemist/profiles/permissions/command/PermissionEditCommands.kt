...
class PermissionEditCommands : BaseCommand() {
    ...
    fun manualPermEdit(
        ...
        @Name("permission") perm: String,
        @Name("scope") scope: String,
        ...
        target.additionalPermissions!!.add(
            ApplicablePermission(
                ...
                if (!duration.equals("perm", ignoreCase = true) && !duration.equals(
                        "permanent",
                        ignoreCase = true
                    )
                ) TimeUtil.parseTime(duration) * 1000L else Long.MAX_VALUE,
                perm.lowercase(),
            )
        )
    ...
    fun onListPermission(sender: CommandSender, @Name("target") target: GameProfile) {
        val output = object : PaginatedOutput<ApplicablePermission>() {
            ...
            override fun format(result: ApplicablePermission, resultIndex: Int): String {
                return Chat.format(
                    "&7* &6${result.node}&7, &eDuration&7: &f${TimeUtil.formatDuration(result.duration)}&7, &eScopes&7: &7[${
                        result.scopes.joinToString(
                            ", "
                        )
                    }]"
                )
    ...
    fun onDeletePermission(sender: CommandSender, @Name("target") target: GameProfile, @Name("node") node: String) {
        ...
        val perms = target.additionalPermissions.also { permissions ->
